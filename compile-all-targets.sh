...
echo -e "${H1}Compilation of all targets for locmess $version${EH}"
...
cargo build --release
strip target/release/locmess
mkdir build/x86_64-linux/
cp target/release/locmess build/x86_64-linux/

# build a musl version
echo -e "${H2}Compiling the MUSL version${EH}"
cross build --release --target x86_64-unknown-linux-musl
mkdir build/x86_64-unknown-linux-musl
cp target/x86_64-unknown-linux-musl/release/locmess build/x86_64-unknown-linux-musl
...
cp target/x86_64-pc-windows-gnu/release/locmess.exe build/x86_64-pc-windows-gnu/
